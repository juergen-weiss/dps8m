...
//}
    ...
    putbits36_1 (& words[5], 27, cpu_p->cycle == INTERRUPT_cycle ? 1 : cpu_p->cu.rfi);
    putbits36_1 (& words[5], 28, cpu_p->cu.its);
    putbits36_1 (& words[5], 29, cpu_p->cycle == INTERRUPT_cycle ? 1 : cpu_p->cu.FIF);
	...
	rewrite_table[] =
	  {
	    ...
	    { { 0000000404202, 0000000000041, 0000000000100, 0000000000000, 0002000002200, 0002000000500, 0001777235000, 0001777235000 },
	      { 0000000400202, 0000000000041, 0000000000100, 0000000000000, 0002000002200, 0002000000500, 0001777235000, 0001777235000 },
	...
	if ((ci->info->flags & RMW) == RMW)
	  {
	      ...
	      if (cpu_p->iefpFinalAddress != cpu_p->rmw_address)
		sim_warn("executeInstruction: write addr changed %o %d\n", cpu_p->iefpFinalAddress, cpu_p->rmw_address);
...
//        case x0 (0673):  // tsp7
	  ...
	  if (get_bar_mode (cpu_p))
	    ...
	    {
	      cpu_p->TPR.CA = get_BAR_address (cpu_p, cpu_p->TPR.CA);
	      read_tra_op (cpu_p);
	      CLR_I_NBAR;
	    }
          return CONT_TRA;
...
elapsedtime ();
 sim_printf (" rcu to %05o:%06o  PSR:IC %05o:%06o\r\n",  (cpu_p->Yblock8[0]>>18)&MASK15, (cpu_p->Yblock8[4]>>18)&MASK18, cpu_p->PPR.PSR, cpu_p->PPR.IC);
    ...
    if_sim_debug (DBG_FAULT, & cpu_dev)
      {
	dump_words(cpu_p, cpu_p->Yblock8);
        //for (int i = 0; i < 8; i ++)
        //  {
...
// clear what it does to the MC data to signal RFI behavior.
    ...
    word5 fi_addr = getbits36_5  (cpu_p->Yblock8[1], 30);
